...
3. Install `requirements-dev.txt` into the app's Python environment and run `python -m pytest tests -q`. With Node.js 24, run `node --test tests/test_passage_editor.js`; Node.js is needed for development checks only. Test a new writable folder, a folder containing spaces, a computer without a configured `runtime.json`, and the browser fallback. For a release, also test full model setup, one real recording, speaker naming, whole-passage edits with bold, italic, underline, multiple lines and a passage longer than 900 characters, note generation with a local model, Word and Markdown exports, Trash and recovery.
4. Build the archive using the explicit file allowlist:

   ```powershell
   python tools/package_release.py --output dist/MeetingStudio-1.1.1-Windows-x64.zip
   python tools/package_release.py --source-only --output dist/MeetingStudio-1.1.1-source.zip
   ```

   The script refuses missing inputs or an existing output file and writes a SHA-256 checksum beside each ZIP. It includes `languages.py`, source, desktop binaries for the Windows ZIP, installer scripts and all bundled third party notices. It never traverses `data` or `.venv`. Review the ZIP member list before sharing.
